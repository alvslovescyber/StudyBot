...
struct TermGlanceTests {
    ...
    private func glance(
        ...
        return TermGlance(
            term: term, today: day, weekOfTerm: calendar.weekOfTerm(day),
            slots: SessionCatalog.slots(in: events),
    ...
    func weekTwo() throws {
        ...
        let noted = Session(
            sync: .new(id: monday.id, at: Self.t0), title: monday.title, date: monday.day.date,
            liveNotes: "- sets")
        let attendedByHours = OTJEntry(
            sync: .new(at: Self.t0), date: blockDay.day.date, hours: 7, category: .workshop,
            description: "Block 1",
        ...
        let old = Evidence(
            ...
            source: .workProject)
        let glance = try glance(
            on: today, sessions: [noted], evidence: [evidence, old], hours: [attendedByHours])
